...
class AccountEdit extends React.Component {
  constructor() {
    ...
    this.editMediaOrder = this.editMediaOrder.bind(this);
  ...
  async handleMediaImageChange(event) {
    ...
    if (
      this.props.user.user.mediaImages.length >= StaticData.MAX_MEDIA_IMAGES
    ) {
      alert(
        `Cannot have more than ${StaticData.MAX_MEDIA_IMAGES} images in gallery`
      );
      return;
    }
    const image = event.target.files[0];
  ...
  onDeleteMediaImage(event) {
    if (event.target.hasAttribute("data-index")) {
      ...
      let newOrder = [];
      for (const [key, value] of Object.entries(this.state.mediaOrder)) {
        newOrder[value] = parseInt(key);
      }

      newOrder.splice(
        newOrder.findIndex((pos) => pos == targetIndex),
        1
      );
      for (const [idx, position] of newOrder.entries()) {
      ...
      this.setState({
        mediaOrder: Object.fromEntries(newOrder.map((pos, idx) => [pos, idx])),
      });
    }
  }
  ...
  onSaveMediaOrder() {
    let mediaOrderData = [];
    for (const [key, value] of Object.entries(this.state.mediaOrder)) {
      mediaOrderData[value] = parseInt(key);
    }

    this.props.saveMediaOrder(mediaOrderData);
  }

  toggleEditImages() {
    if (this.state.editMode) {
      this.onSaveMediaOrder();
    }

    this.setState({ editMode: !this.state.editMode });
  }

  initializeMediaOrder() {
    if (!this.state.mediaOrderInitialized) {
      let user = this.props.user.user;
      this.setState({
        mediaOrder: Object.fromEntries(
          user.mediaOrder.map((pos, idx) => [pos, idx])
        ),
        mediaOrderInitialized: true,
      });
    }
  }

  editMediaOrder(event) {
    if (this.state.editMode) {
      let clickedPos = event.target.getAttribute("data-index");
      let clickedIndex = this.state.mediaOrder[clickedPos];

      let newOrder = Object.assign({}, this.state.mediaOrder);
      for (const [key, value] of Object.entries(newOrder)) {
        if (key != clickedPos && value > clickedIndex) {
          newOrder[key]--;
        }
      }
      newOrder[clickedPos] = this.props.user.user.mediaOrder.length - 1;

      this.setState({ mediaOrder: newOrder });
    }
  }
    ...
    if (this.props.user.user && !isLoading) {
      ...
      if (user.type === "client") {
        ...
        userData = (
          <div>
            ...
            <input
              ...
              hidden="hidden"
            />
            <a href={`/user/${user.userHandle}`} className="invisible-link">
              <p className="full-name">{user.fullName}</p>
            </a>
            <a href={`/user/${user.userHandle}`}>
              <p className="user-handle">@{user.userHandle}</p>
            </a>
            <p className="user-info">
              <EmailIcon fontSize="small" />
        ...
        user.mediaOrder.forEach((position) => {
          ...
          imageGallery.push(
            <Grid key={position} sm={6} xs={12}>
              <div className="gallery-image-container">
                <Badge
                  badgeContent={this.state.mediaOrder[position] + 1}
                  color="primary"
                  invisible={!this.state.editMode}
                >
                  <img
                    className="gallery-image"
                    data-index={position}
                    onClick={this.editMediaOrder}
                    src={imageURL}
                    alt="Gallery"
                  />
                </Badge>
                ...
                {this.state.editMode ? (
                  <IconButton
                    data-index={position}
                    onClick={this.onDeleteMediaImage}
                  >
                    <CloseIcon data-index={position} />
                  </IconButton>
                ) : null}
              </div>
            </Grid>
        ...
        galleryContent = (
          <div>
            ...
            <p className="lightText">
              {" "}
              Choose images that you would like to display on your profile here.
              {" "}
            </p>
            ...
            {/* <p className="lightText">
              {" "}
              Choose images that you would like to display on your profile here.
              Click on the "Edit Images" button to edit your image gallery.{" "}
            </p> */}

            {/* <p className="lightText">
              {" "}
              You can click on any image to set your desired display order or
              click on the "X" button below each image to delete that image.
            </p> */}
            <div class="divider" />

            <Grid container>{imageGallery}</Grid>

            <div class="divider" />
            <input
              type="file"
              id="imageGalleryInput"
              onChange={this.handleMediaImageChange}
              hidden="hidden"
            />
            <Button
              variant="outlined"
...
AccountEdit.propTypes = {
  ...
  saveMediaOrder: PropTypes.func.isRequired,
};
